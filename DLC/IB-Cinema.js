...
+'.ci-ui{position:absolute;inset:0;z-index:3;opacity:1;transition:opacity 0.4s}.ci-stage.hideui .ci-ui{opacity:0;pointer-events:none}.ci-stage.hideui{cursor:none}'+'/* [v10] 放映性能：整页毛玻璃与控制条实时模糊压在 1080p 视频上会掉帧，放映中改为实色半透明 */'+'.ci-stage.hideui .ci-ui{visibility:hidden;transition:opacity 0.4s,visibility 0s 0.4s}'+'body.ci-on #page-cinema.active::before{backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(8,14,30,0.9)!important}'+'body.ci-on .ci-top{backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(8,14,30,0.72)!important}'
...
async function migrate(){try{if(await ctx.storage.get(K.mig))return;var all=await ctx.storage.list('');var n=0;
  for(var i=0;i<all.length;i++){var r=all[i];if(!r||!r.k||!r.v)continue;var k=String(r.k);if(k.indexOf('film_')!==0)continue;var v=r.v;
    if(v.isNet){if(!v.sourceUrl)continue}/* [net] 1.12 的网络视频记录（film_net_*，带 isNet / sourceUrl / threadId）一并搬 */
    else if(v.file===undefined||v.threadId!==undefined)continue;/* 手机端记录有 threadId / name，桌面端记录有 file / thread：只搬桌面端自己写的 */
    var key=k.slice(5);
    if(v.isNet){if(!v.thread&&v.threadId!==undefined)v.thread=v.threadId;if(!v.file)v.file=v.name||v.sourceUrl}/* [net] 旧网络记录字段对齐 2.x（thread / file） */
    if(!(await ctx.storage.get(K.film+key)))await ctx.storage.set(K.film+key,v);await ctx.storage.remove(k);
    var sm=await ctx.storage.get('sum_'+key);if(sm&&(sm.t!==undefined||sm.upTo!==undefined)){if(sm.t===undefined)sm={text:String(sm.text||''),t:Number(sm.upTo)||0,updated:0,full:false};await ctx.storage.set(K.sum+key,sm);await ctx.storage.remove('sum_'+key)}/* [net] 1.12 的梗概是 {text,upTo}，搬时对齐成 {text,t} */
...
async function renderLib(){S.view='lib';var pg=page();if(!pg)return;if(!S.cfg)await loadCfg();pg.classList.remove('play');pg.classList.remove('focus');document.body.classList.remove('ci-on');document.body.classList.remove('ibr-col-off');
  ...
  $('ci-left').innerHTML='<div class="ci-hd"><b>观影室</b><i>Cinema</i></div>'
    ...
    +'<div class="ci-net"><div class="ci-netrow"><input class="ci-net-in" id="ci-net-url" type="url" inputmode="url" placeholder="网络视频：链接、播放页，或直接输片名" autocomplete="off" spellcheck="false"><button class="btn" id="ci-netbtn">打开</button></div><small>支持 MP4 / WebM / M3U8 直链与网页播放页；直接输入片名会在服务器上搜片（archive.org 公版片库）。DRM、需登录的站不支持。历史里的网络片点开会自动重新解析。</small></div>'/* [net] 网络视频入口（自 1.12 移植） */
  ...
  var nb=$('ci-netbtn');if(nb)nb.addEventListener('click',function(){var nc=$('ci-net-url');var u=nc&&nc.value.trim();if(!u){ctx.ui.toast('请输入网络视频地址');return}pickNet(u)});/* [net] */
  var nin=$('ci-net-url');if(nin)nin.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();var u=this.value.trim();if(u)pickNet(u)}});/* [net] 回车即解析 */
...
function pickFor(rec){if(rec.isNet){reNet(rec);return}/* [net] 网络记录：重新解析源地址（令牌过期自愈），不弹文件选择器 */var inp=fileInput();inp.value='';inp.onchange=function(){var f=inp.files&&inp.files[0];if(!f)return;S.file=f;if(f.name!==rec.file||f.size!==rec.size)ctx.ui.toast('和记录里的文件名或大小不一样，仍按这个文件放');stageRec(rec,false)};inp.click()}
...
function unstage(){S.ready=null;S.file=null;S.playNet=null;S.probeId++;drawHero(S.top,false);syncTop()}/* [net] 连网络流一起清 */
...
function refile(){if(!S.ready){ctx.ui.toast('先选一部片');return}var rec=S.ready.rec,isNew=S.ready.isNew;if(rec.isNet){var nc=$('ci-net-url');if(nc){nc.value=rec.sourceUrl||'';nc.focus();ctx.ui.toast('改好地址再点「打开」：同源会接着这条记录，新源则新开一条')}return}/* [net] */var inp=fileInput();inp.value='';inp.onchange=function(){var f=inp.files&&inp.files[0];if(!f)return;
...
async function startStaged(){if(!S.ready)return;var rec=S.ready.rec;if(!S.file&&!S.ready.net){ctx.ui.toast('先选好视频文件');return}/* [net] 网络视频没有本地文件 */if(!ctx.chat.cfg(rec.cfgId)){ctx.ui.toast('这位 TA 已不在');return}/* 2.3.2：先验 TA，再清待放映态（原来先清再进 play，TA 不在时银幕停在待放映的样子却点不动） */var c=SUBS[rec.key]||null;S.subs=c?c.cues:[];if(S.subs.length)rec.sub=c.name;/* 这次没带字幕也留着上次的字幕名，银幕上好提示 */S.playNet=(S.ready.rec.direct&&S.ready.rec.srcUrl)?S.ready.rec.srcUrl:S.ready.net||null;S.playProxy=S.ready.net||null;S.pxTried=false;/* [v8] 直连优先，云代理回落 */S.playKind=(S.ready.net&&(S.ready.rec.netKind||(/\.m3u8($|\?)/i.test(S.ready.rec.sourceUrl||'')?'hls':'video')))||null;S.ready=null;S.probeId++;await play(rec)}/* [net] 带上待放映的网络流地址与类型 */
/* ═══ 放映 ═══ */
async function play(rec){var cfg=ctx.chat.cfg(rec.cfgId);if(!cfg){ctx.ui.toast('这位 TA 已不在');return}
  ...
  var v=video();v.playbackRate=1;/* [net] 本地文件原路径不变；网络流直挂代理地址，HLS 走 hls.js（类型来自解析结果的 stream_type，代理流地址不带 .m3u8 后缀） */
  if(S.file){S.url=URL.createObjectURL(S.file);v.src=S.url}
  else{S.url=S.playNet||'';hlsStop();if(S.playKind==='hls'||/\.m3u8($|\?)/i.test(S.url)){v.removeAttribute('src');hlsAttach(S.url)}else{v.src=S.url}}
  /* 右栏：自带消息列表的频道栏 */
...
function sysBlock(){var r=S.rec;if(!r)return '';
  ...
  return '【观影室】你和“'+un()+'”正在 Internal Beyond 的观影室里一起看'+(r.isNet?'一部网络视频':'一部本机视频')+'《'+r.title+'》：对方在放，你陪着看。对方每条消息里会带一段「系统随消息附上的观影状态」（在【用户当前消息】之前）——播放点之前的字幕（如有）、进度、前情梗概、时间轴笔记，以及消息里若附了画面才有的那一帧'+(S.see==='turn'?'（默认每条消息附此刻一帧，随消息临时附上，不留在聊天记录里）':(S.see==='snap'?'（只在对方留影时附）':(S.see==='off'?'（对方没开画面，你看不到画面）':'')))+'；那不是对方说的话，对方说的话在【用户当前消息】之后。播放点之后的剧情你不知道，不预告、不猜结局、不引用没给你的台词，没给你的画面不描述。回复像坐在旁边看片的人随口说的话，不总结不分析。'}
...
function tailText(t,d,frame){var r=S.rec;var mins=Math.max(1,Math.round((Date.now()-S.enteredAt)/60000));var rs=subsBefore(t,S.subN);
  ...
  s+='\n'+bound(frame)+'\n[说明] 以上是这一轮你知道的全部：'+(S.subs.length?'字幕只到播放点为止，':'')+'后面的剧情你不知道，不预告、不猜。像坐在旁边一起看片的人那样接对方在【用户当前消息】里的那句话。';
  return s}
...
function bindPlayer(){var v=video(),st=stage();
  ...
  v.addEventListener('error',function(){if(S.rec&&S.rec.isNet){hlsStop();ctx.ui.toast('网络视频打不开：直链可能已过期、源站拒绝跨域或编码不受支持；回视频库点这条记录会自动重新解析');return}/* [net] */ctx.ui.toast('这个文件放不出来：'+(v.error&&v.error.message||'浏览器不支持它的编码')+'。换一个 mp4 / webm 试试')});
  function toggle(){if(S.rec&&S.rec.isNet&&S.playKind==='hls'&&v.readyState===0)return;/* [net] HLS 数据未就绪时忽略点击，避免中断 hls.js 起播 */if(v.paused)v.play().catch(function(){});else v.pause()}
...
/* ═══ [net] 网络视频（自 1.12+解析版移植，适配 2.6.3 银幕即入口架构）═══
   入口在左栏：直链 / 播放页 → POST /film-matinee/url/open（8771 代理解析，同源流、带 Range）
   → 待放映（离屏探海报 / 时长）→ 放映（MP4/WebM 直挂代理流；M3U8 用 DLC/vendor/hls-1.6.13.min.js）
   历史里的网络片点卡片＝重新解析一次（令牌过期自愈）；记录存 isNet + sourceUrl。 */
(function(){
  var st=document.createElement('style');st.id='ib-cinema-net-css';
  st.textContent='#page-cinema .ci-net{margin:10px 0 2px;display:flex;flex-direction:column;gap:6px}'
    +'#page-cinema .ci-netrow{display:flex;gap:6px;align-items:stretch}'
    +'#page-cinema .ci-net-in{flex:1;min-width:0;box-sizing:border-box;padding:8px 10px;border-radius:9px;border:1px solid rgba(120,150,200,.32);background:rgba(127,150,190,.10);color:inherit;font:inherit;font-size:.82rem;outline:none}'
    +'#page-cinema .ci-net-in:focus{border-color:rgba(114,168,216,.72)}'
    +'#page-cinema .ci-net small{opacity:.66;font-size:.72rem;line-height:1.65}'
    +'body:not(.theme-infernal) #page-cinema .ci-net-in{background:rgba(255,255,255,.5);border-color:rgba(96,140,196,.35)}';
  document.head.appendChild(st);
})();
var _hls=null,_hlsScript=null,_hlsTimer=null,_hlsGen=0;
var _hlsSrc=new URL('vendor/hls-1.6.13.min.js',document.currentScript&&document.currentScript.src||new URL('DLC/IB-Cinema.js',document.baseURI).href).href;
function hlsStop(){clearTimeout(_hlsTimer);_hlsTimer=null;if(_hlsScript){_hlsScript.onload=_hlsScript.onerror=null;_hlsScript.remove();_hlsScript=null}if(_hls){var old=_hls;_hls=null;try{old.destroy()}catch(e){}}_hlsGen++}
function hlsAttach(url){var v=video();if(!v)return;var gen=_hlsGen;
  function dead(){return gen!==_hlsGen||!v.isConnected}
  function fail(msg){if(dead())return;hlsStop();ctx.ui.toast(msg)}
  function ready(){if(dead())return;var H=window.Hls;
    if(H&&H.isSupported()){/* [net] Chrome/Edge/Firefox 一律优先 hls.js（部分 Chromium canPlayType 对 HLS 报 maybe 却放不动）；原生 HLS 只作兜底 */
      try{var _hc={maxBufferLength:60,maxMaxBufferLength:120,startFragPrefetch:true};
        try{/* [net-cache] 分片浏览器缓存：按源站地址(u参数)存 Cache API，跨会话有效——重看/回拖不再过云 */
          if(window.caches&&caches.open){var _BL=H.DefaultConfig.loader;
            var _sk=function(u){var m=/[?&]u=([^&]+)/.exec(String(u||''));try{return m?decodeURIComponent(m[1]):String(u||'')}catch(e){return String(u||'')}};
            var _FCL=function(k){_BL.call(this,k)};
            _FCL.prototype=Object.create(_BL.prototype);_FCL.prototype.constructor=_FCL;
            _FCL.prototype.load=function(ctx,config,cb){
              /* [net-cache] v1.6 分片上下文无 type 且以 rangeStart/rangeEnd 拉取：缓存键=源站地址(u)，
                 只拦「非中段 Range」的整片请求；命中直出，未命中走默认加载并回写缓存 */
              var self=this,url=(ctx&&ctx.url)||'';
              var _mk=/[?&]u=([^&]+)/.exec(url);
              var _key=null;try{_key=_mk?decodeURIComponent(_mk[1]):null}catch(e){}
              if(!_key)_key=url;/* [v8] 直连地址本身稳定，键=完整 URL */
              var _cacheable=!!_key&&window.caches&&caches.open&&!(ctx&&ctx.rangeStart!=null&&ctx.rangeStart>0);
              if(_cacheable){
                caches.open('ib-film-v1').then(function(c){return c.match(_key).then(function(hit){return [c,hit]})}).then(function(rr){
                  var cc=rr[0],hit=rr[1];
                  if(hit&&hit.ok){return hit.clone().arrayBuffer().then(function(buf){
                    var now=performance.now();
                    if(!self.stats)self.stats={};if(!self.stats.loading)self.stats.loading={};
                    self.stats.loading.start=self.stats.loading.start||now;self.stats.loading.first=now;self.stats.loading.end=now;
                    window.__filmCacheHit=(window.__filmCacheHit||0)+1;
                    try{cb.onSuccess({url:url,data:buf},self.stats,ctx)}catch(e){}
                  })}
                  var _os=cb.onSuccess;
                  cb.onSuccess=function(res,st,cx,nr){try{if(res&&res.data)cc.put(_key,new Response(res.data,{headers:{'Content-Type':'video/mp2t'}}))}catch(e){}return _os.call(cb,res,st,cx,nr)};
                  _BL.prototype.load.call(self,ctx,config,cb);
                }).catch(function(){_BL.prototype.load.call(self,ctx,config,cb)});
                return;
              }
              _BL.prototype.load.call(this,ctx,config,cb);
            };
            _hc.fLoader=_FCL;_hc.pLoader=_FCL;_hc.loader=_FCL;
          }
        }catch(e){}
        var h=new H(_hc);_hls=h;clearTimeout(_hlsTimer);_hlsTimer=null;/* [fix] 双保险；[v5] 客户端缓冲带 60s+清单期预取首片 */
        h.on(H.Events.MANIFEST_PARSED,function(){if(!dead()&&_hls===h){try{var p2=v.play();if(p2&&p2.catch)p2.catch(function(){})}catch(e){}}});
        h.on(H.Events.ERROR,function(ev,data){if(!dead()&&_hls===h&&data&&data.fatal){/* [v8] 直连失败自动回落云代理一次 */
          if(S.rec&&S.rec.direct&&S.playProxy&&!S.pxTried){S.pxTried=true;S.rec.direct=false;S.playNet=S.playProxy;hlsStop();var v2=video();if(v2)v2.removeAttribute('src');hlsAttach(S.playProxy);ctx.ui.toast('直连不畅，已切换云加速通道');return}
          fail('HLS 视频加载失败：请换一个直链，或回视频库点这条记录重新解析')}});
        h.loadSource(url);h.attachMedia(v);return}catch(e){fail('HLS 播放器启动失败');return}}
    if(v.canPlayType('application/vnd.apple.mpegurl')){v.src=url;try{var p0=v.play();if(p0&&p0.catch)p0.catch(function(){})}catch(e){}return}
    fail('当前浏览器不支持 HLS 播放，请换用 MP4 直链')}
  if(window.Hls){ready();return}
  var _sN=0;
  var sc=document.createElement('script');_hlsScript=sc;sc.src=_hlsSrc;
  sc.onload=function(){clearTimeout(_hlsTimer);_hlsTimer=null;/* [fix] 脚本已到手，15s 装载保险必须拆除——否则正常播放 15 秒后被误杀 */if(!dead())ready()};
  sc.onerror=function(){/* [v9] 脚本经弱网加载偶发失败：自动换缓存戳重试 2 次再放弃 */
    if(!dead()&&_sN<2){_sN++;var sc2=document.createElement('script');_hlsScript=sc2;sc2.src=_hlsSrc+(_hlsSrc.indexOf('?')>=0?'&':'?')+'r='+_sN;sc2.onload=sc.onload;sc2.onerror=sc.onerror;document.head.appendChild(sc2);return}
    fail('HLS 播放器加载失败（网络波动），请再点一次播放重试')};
  _hlsTimer=setTimeout(function(){fail('HLS 播放器加载超时，请重新打开')},15000);
  document.head.appendChild(sc)}
function probeNet(stream,rec,done){/* 与官方 probe() 同一套监听，只是源是代理流地址（同源、带 Range），不走 objectURL */
  if(rec&&rec.netKind==='hls'){done('',0,0);return}/* [net] HLS 清单不能喂普通 video（必须 hls.js）；时长由解析端汇总 EXTINF 提供，海报待放映后自动抓 */
  var id=++S.probeId;var v=document.createElement('video');v.muted=true;v.playsInline=true;v.preload='auto';var fin=false;
  function end(p,w,h){if(fin)return;fin=true;try{v.removeAttribute('src');v.load()}catch(e){}if(id!==S.probeId)return;done(p,isFinite(v.duration||0)?(v.duration||0):0,w,h)}
  v.addEventListener('loadedmetadata',function(){var d=v.duration||0;var t=(d&&isFinite(d))?Math.min(Math.max(1.5,d*0.08),Math.max(0,d-0.5)):0;try{v.currentTime=t}catch(e){end('',v.videoWidth||0,v.videoHeight||0)}});
  v.addEventListener('seeked',function(){try{var w=Math.min(1280,v.videoWidth||0);if(!w)return end('',0,0);var h=Math.round(v.videoHeight*w/v.videoWidth);var c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(v,0,0,w,h);end(c.toDataURL('image/jpeg',0.82),v.videoWidth,v.videoHeight)}catch(e){end('',v.videoWidth||0,v.videoHeight||0)}});
  v.addEventListener('loadeddata',function(){try{if(!(v.duration&&isFinite(v.duration)&&v.duration>2)){var w=Math.min(1280,v.videoWidth||0);if(!w)return;var h=Math.round(v.videoHeight*w/v.videoWidth);var c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(v,0,0,w,h);end(c.toDataURL('image/jpeg',0.82),v.videoWidth,v.videoHeight)}}catch(e){}});
  v.addEventListener('error',function(){end('',0,0)});
  setTimeout(function(){end('',v.videoWidth||0,v.videoHeight||0)},9000);
  v.src=stream}
function stageNet(rec,isNew,stream){S.ready={rec:rec,isNew:isNew,net:stream};S.file=null;drawHero(rec,true);syncTop();var mid=$('ci-mid');if(mid){try{mid.scrollTo({top:0,behavior:'smooth'})}catch(e){mid.scrollTop=0}}
  probeNet(stream,rec,function(p,d,w,h){if(!S.ready||S.ready.rec!==rec)return;if(d)rec.dur=d;if(p){rec.poster=p;rec.posterW=Math.min(1280,w||0)}if(w)rec.vw=w;if(h)rec.vh=h;drawHero(rec,true)})}
async function reNet(rec){var u=rec.sourceUrl;if(!u){ctx.ui.toast('这条记录没有源地址，删掉后请重新添加');return}
  ctx.ui.toast('正在重新解析网络视频…');
  try{var resp=await fetch('/film-matinee/url/open',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:u})});
    var data=await resp.json().catch(function(){return{}});
    if(!resp.ok||!data.film)throw new Error(data.error||('网页视频解析失败（'+resp.status+'）'));
    var stream=new URL(data.film.stream_path,location.href).href;
    rec.netKind=(String(data.film.stream_type||'')==='hls'||/mpegurl/i.test(String(data.film.mime||'')))?'hls':'video';
    stageNet(rec,false,stream);ctx.ui.toast('解析好了，点银幕开始放')}
  catch(e){ctx.ui.toast('网络视频重新解析失败：'+String((e&&e.message)||e))}}
async function pickNet(url){
  var clean=String(url||'').trim(),parsed=null;
  if(/^https?:/i.test(clean)){/* [net-search] 像链接才按链接校验；否则当片名交给服务器搜索（archive.org 公版片库） */
    try{parsed=new URL(clean)}catch(e){ctx.ui.toast('请输入完整的 HTTP(S) 视频或播放页地址');return}
  if(!/^https?:$/.test(parsed.protocol)||parsed.username||parsed.password){ctx.ui.toast('只支持不含账号密码的 HTTP / HTTPS 地址');return}
  if(window.location&&window.location.protocol==='https:'&&parsed.protocol==='http:'){ctx.ui.toast('当前页面使用 HTTPS，请换用 HTTPS 地址');return}
    clean=parsed.href;
  }
  var btn=$('ci-netbtn');if(btn)btn.disabled=true;
  try{
    ...
    var direct=!!film.direct,srcUrl=film.src_url?String(new URL(film.src_url,location.href).href):null;/* [v8] 直连模式：浏览器直取源站，不过云 */
    var kind=(String(film.stream_type||'')==='hls'||/mpegurl/i.test(String(film.mime||'')))?'hls':'video';/* [net] 代理流地址不带 .m3u8，类型必须随解析结果走 */
    var name=film.title||(parsed?parsed.pathname.split('/').pop():'')||clean.slice(0,60)||'远程视频';
    var fid=($('ci-ta')&&$('ci-ta').value)||S.cfgId;
    if(!fid){ctx.ui.toast('先到 API 页添加一位 TA');return}
    var base='net_'+hash(clean),rec=null;
    if(S.ready&&S.ready.rec&&S.ready.rec.isNet&&String(S.ready.rec.key).indexOf(base+'_')===0)rec=S.ready.rec;
    if(rec){rec.sourceUrl=clean;rec.netKind=kind;rec.direct=direct;rec.srcUrl=srcUrl;stageNet(rec,false,stream);ctx.ui.toast(direct?'还是这条记录，已换上新解析的流（直连源站）':'还是这条记录，已换上新解析的流')}
    else{var key=base+'_'+Date.now().toString(36);
      rec={key:key,title:name,file:name,size:0,dur:0,sub:'',cfgId:fid,thread:'',done:false,notes:[],poster:'',sessions:0,totalMs:0,lastT:0,lastTs:0,created:Date.now(),updated:Date.now(),isNet:true,sourceUrl:clean,netKind:kind,direct:direct,srcUrl:srcUrl};
      stageNet(rec,true,stream)}
  }catch(e){ctx.ui.toast(String((e&&e.message)||e||'网页视频解析失败'))}
